...
if (count($_POST) === 6 && isset($_POST['username'], $_POST['password'], $_POST['email'], $_POST['age'], $_POST['name'], $_POST['surname'])) {
	...
	$DB_Age			= $_POST['age'];
	$DB_Name		= $db->quote($_POST['name']);
	$DB_Surname		= $db->quote($_POST['surname']);
	...
	if (!isset($ERROR)) {
		...
		$db->query("INSERT INTO users set username=$DB_Username, password=$DB_Password, email=$DB_Email, age=$DB_Age, name=$DB_Name, surname=$DB_Surname, hash='$vHash'");
		$ERROR	= "success";
